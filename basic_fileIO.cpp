...
namespace basic_fileIO {
	...
	void save_as_bmp(
		const std::string& path, const double* src,
		size_t width, size_t height
	) {
		auto folder_path = fs::path(path).remove_filename();
		if (!fs::exists(folder_path)) {
			fs::create_directories(folder_path);
		}

		const auto first = src;
		const auto last = std::next(src, width * height - 1); // - 1 so last points to begin + width * height, one behind the last element
		...
		save_as_bmp(path, result.data(), width, height);
	}

	void read_from_bmp(
		const std::string& path, byte* dst,
		size_t width, size_t height
	
	) {
		
		if (!fs::exists(path)) {
			std::string message = "read_from_bmp: Cannot open file \"" + path + "\"";
	...
	void load_phase_correction(byte* phase_correction_ptr,
